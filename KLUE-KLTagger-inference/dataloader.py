...
class KlueDpDataLoader(object):
    ...
    def collate_fn(self, batch):
        ...
        pos_padding_idx2 = None if self.args.no_pos else len(get_pos_labels())
        pos_padding_idx3 = None if self.args.no_pos else len(get_pos_labels())
        ...
        for batch_id in range(batch_size):
            ...
            if not self.args.no_pos:
                ...
                pos_ids2[batch_id][0] = pos_padding_idx2
                pos_ids2[batch_id][1:] = token_pos_ids2[head_id]
                pos_ids2[batch_id][torch.sum(bpe_head_mask) + 1:] = pos_padding_idx2

                pos_ids3[batch_id][0] = pos_padding_idx3
                pos_ids3[batch_id][1:] = token_pos_ids3[head_id]
                pos_ids3[batch_id][torch.sum(bpe_head_mask) + 1:] = pos_padding_idx3
